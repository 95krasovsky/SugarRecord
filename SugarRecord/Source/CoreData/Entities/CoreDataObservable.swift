...
public class CoreDataObservable<T: NSManagedObject>: RequestObservable<T>, NSFetchedResultsControllerDelegate where T:Equatable {
    ...
    public func controller(_ controller: NSFetchedResultsController<NSFetchRequestResult>, didChange anObject: Any, at indexPath: IndexPath?, for type: NSFetchedResultsChangeType, newIndexPath: IndexPath?) {
        switch type {
        case .delete:
            self.batchChanges.append(.delete(indexPath![1], anObject as! T))
        case .insert:
            self.batchChanges.append(.insert(newIndexPath![1], anObject as! T))
        case .update:
            self.batchChanges.append(.update(indexPath![1], anObject as! T))
        default: break
        }
    }
